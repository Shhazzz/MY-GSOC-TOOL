...
async function loadGitHubData(config) {
    ...
    try {
        // ALWAYS fetch live data instead of stale file
        console.log("Skipping static file, using live API");
        }
    } catch (e) {
...
function renderBlogPosts(posts) {
    ...
    if (posts && posts.length > 0) {
        blogList.innerHTML = posts.map((post, index) => `
            <div class="p-4 border border-gray-100 dark:border-gray-700 rounded-lg hover:shadow-md transition-shadow bg-white dark:bg-gray-700/50 group">                <h3 class="font-semibold text-lg text-gray-800 dark:text-white group-hover:text-primary transition-colors mb-2">${post.title}</h3>
